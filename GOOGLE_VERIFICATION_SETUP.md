...
1. 사용자가 구글 로그인 시도
2. "인증 코드가 생성되었습니다. 관리자에게 문의하여 인증 코드를 받으세요." 메시지 표시
3. 사용자가 관리자에게 연락 (이메일, 전화, 메신저 등)
4. 관리자가 백엔드 로그 또는 API로 인증 코드 확인
5. 관리자가 사용자에게 6자리 인증 코드 전달
6. 사용자가 인증 코드 입력 페이지에서 코드 입력
7. 인증 완료 → 자동 로그인
